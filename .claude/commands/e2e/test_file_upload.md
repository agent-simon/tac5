# E2E Test: Upload CSV File via Browse Files

Test uploading a custom CSV file through the Browse Files button and verifying the table appears correctly in Available Tables.

## User Story

As a user
I want to upload my own CSV file using the Browse Files button
So that I can query my custom data using natural language
...
4. Create a temporary CSV file at `/tmp/test_employees.csv` with the following content:
   ```
...
9. Upload the file `/tmp/test_employees.csv` via the hidden file input (`#file-input`)
...
13. **Verify** the table name shown in `.table-name` is "test_employees"
14. **Verify** the table info shown in `.table-info` contains "3 rows" and "4 columns"
15. **Verify** column tags (`.column-tag`) are displayed showing the column names: "name", "age", "department", "salary"
16. Take a screenshot of the Available Tables section showing the uploaded table
17. Clean up the temporary CSV file at `/tmp/test_employees.csv`

## Success Criteria
- Upload Data modal opens when the button is clicked
- Browse Files button and drop zone are visible in the modal
- CSV file is uploaded successfully via the file input
- The uploaded table appears in Available Tables with the correct name "test_employees"
- Row count (3) and column count (4) are displayed correctly
- Column names and types are shown as column tags
